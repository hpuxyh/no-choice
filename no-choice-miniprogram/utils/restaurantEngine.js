...
async function loadRestaurantDeck({ modeName, choice, coords, setLoading, toast, avoidCardKeys = [] }) {
  ...
  const cardOptions = { ...searchOptions, userCoords: center, searchCoords: result.coords, destination, meetup };
  ...
  if (cards.length < TOTAL) {
    const wide = await searchRestaurantsWithFallback(searchCoords, Math.max(radius, 8000), searchPlan.keywords, searchOptions, meetup, { lockSearchCenter });
    let widePois = filterRestaurantPoisWithinSearchRadius(wide.pois, radiusTarget, radius);
...
async function aiRestaurantSearchPlan(choice, coords, { setLoading, toast } = {}) {
  ...
  if (setLoading) setLoading("AI 正在理解中", "正在理解你的场景、人数和位置。");
...
async function geocodeRestaurantLocationHint(hint, coords) {
  ...
  const city = coords && coords.label && !/^(当前|附近|周边|GPS)/.test(coords.label) ? coords.label : "";
  ...
  for (const attemptCity of attempts.filter((item, index, arr) => index === arr.indexOf(item))) {
    try {
      ...
      if (Number.isFinite(lat) && Number.isFinite(lng)) return { lat, lng, latitude: lat, longitude: lng, amap: true, label: cleaned };
    } catch (error) {
...
async function buildRestaurantIntentPreview(choice, coords) {
  ...
  return {
    ...
    details: [
      ...
      { label: "人数", value: restaurantPlanPeopleText(plan, choice, meetup) },
...
function restaurantPlanPeopleText(plan = {}, choice = {}, meetup = null) {
  ...
  if (meetup && Array.isArray(meetup.participants) && meetup.participants.length) {
    ...
    return labels.length ? `${labels.join(" + ")}，共${meetup.participants.length}人` : `共${meetup.participants.length}人`;
  }
  const text = `${choice.question || ""} ${(choice.tags || []).join(" ")}`;
  if (/一人食|一个人|自己吃/.test(text)) return "1人";
  if (/约会|对象|男朋友|女朋友|男友|女友|暧昧/.test(text)) return "你 + 约会对象，共2人";
  if (hasSingleRelationCompanionIntent(choice) || /朋友|同事|同学|客户|聚餐|一起/.test(text)) return "你 + 朋友，共2人";
  const count = String(text).match(/(\d+)\s*人/);
  return count ? `约${count[1]}人` : "1-2人";
}
...
function restaurantPlanMiddleText(plan = {}, choice = {}, destination = null, meetup = null) {
  ...
  if (shouldUseCurrentLocationForMeetup(choice, plan.locationHints || [])) return "取中间点，用你的位置和朋友位置";
